...
<?php
	...
	public function uploadIconPostsMetaBox() {
		...
		<form enctype="multipart/form-data" action="" method="post" name="attachment_manager_upload" id="attachment_manager_upload">
			<?php wp_nonce_field( 'wam_add_icon' ); ?>
	...
	public function handle_actions() {
		if (isset($_GET['page']) && $_GET['page'] == 'attachment_manager') {
			if ( isset($_GET['action']) ) {
				...
				check_admin_referer( 'wam_add_icon' );
				/** WordPress Administration File API */
				require_once( ABSPATH . 'wp-admin/includes/file.php' );

				add_filter( 'upload_dir', array( $this, 'upload_dir' ) );
				$overrides = array( 'test_form' => false );
				$uploaded = wp_handle_upload( $_FILES['wam_add_icon'], $overrides );
	...
	public function upload_dir( $upload_dir ) {
		$upload_dir['path'] = $this->_icon_dir;
		return $upload_dir;
	}
